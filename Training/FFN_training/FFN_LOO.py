from src.AFN_model.FFN_model_archi import FFNModelInitializer
import torch
import json
...
initializer = FFNModelInitializer(config["num_feature_extra_units"], device, config)
...
for i, (train_index, test_index) in enumerate(loo.split(data_tensor)):
    ...
    FFN_model = initializer.create_afn_model().to(device)
    optimizer = optim.Adam(FFN_model.parameters(), lr=config["lr"])
    ...
    for epoch in range(config["epochs"]):
        FFN_model.train()
        optimizer.zero_grad()
        y_train_pred = FFN_model(X_train)
